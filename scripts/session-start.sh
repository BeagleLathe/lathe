...
# `mcp__plugin_lathe_lathe__{search,read,edit,sh}` reach the model,
...
read-only shell command:
  ...
  ToolSearch query: "select:mcp__plugin_lathe_lathe__search,mcp__plugin_lathe_lathe__read,mcp__plugin_lathe_lathe__edit,mcp__plugin_lathe_lathe__sh"

Routing rules (prefer the BeagleLathe tool over the built-in in each case):

  mcp__plugin_lathe_lathe__search   over Grep/Glob — fuses glob + regex + ranked
                                            snippet read into one call; usually removes the
                                            follow-up Read entirely.

  mcp__plugin_lathe_lathe__read     over Read — for source files >300 lines, returns
                                            an AST-truncated view (signatures + types kept,
                                            bodies stubbed). Use mode=full only when you need
                                            every line verbatim.

  mcp__plugin_lathe_lathe__edit     over Edit — even for single edits. Batches multi-
                                            file changes atomically and fuzzy-matches over
                                            whitespace / quote drift that built-in Edit would
                                            reject.

  mcp__plugin_lathe_lathe__sh       over Bash — for read-only inspection (ls, cat,
