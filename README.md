...
**Two commands from zero to working:**

```bash
pip install git+ssh://git@github.com/pbehrens/beagle-lathe.git
beaglelathe install
```

`beaglelathe install` writes `.mcp.json` into the current directory. Open that project in Claude Code and BeagleLathe appears in `/mcp` with eleven tools active.

To configure a specific project directory instead of the current one:

```bash
beaglelathe install /path/to/your-project
```
